...
def extract_website_content(url: str, max_urls: int = 20, show_full_text: bool = True) -> Dict:
    """
        ...
        while status != 'completed' and retry_count < max_retries:
            try:
                ...
                if retry_count >= max_retries:
                    raise ConnectionError(f"Failed to check extraction status after {max_retries} retries: {str(e)}")
